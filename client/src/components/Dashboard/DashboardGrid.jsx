  ...
  if (widgets.length === 0) {
    return (
      <div className="text-center py-16" data-testid="empty-state">
        ...
        <h2 className="text-2xl font-semibold text-foreground mb-2">Build Your Finance Dashboard</h2>
        <p className="text-muted-foreground mb-8 max-w-md mx-auto">
          Create custom widgets by connecting to any finance API. Track stocks, crypto, and market indicators - all in one place.
        </p>
              ...
              <Draggable key={widget.id} draggableId={widget.id} index={index}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`${getGridSpanClass(widget.size)} ${snapshot.isDragging ? 'drag-preview' : ''}`}
